/* ============================================================================
   ...
   <script>...</script> より「後」に以下のように読み込んでください。
     ...
     <script src="dyslexia-support-pack-v0.5.js"></script>
     </body>
   ...
   ▼ 新しく使えるようになるツール
     ...
     ・ボタン読み上げ（🔊／v0.5で新規追加）
       ...
       　各種ダイアログ・DLC一覧・ピン止めパネルなど）の右上に、小さな
       　🔊読み上げバッジが付く。押すとそのボタンの名前や説明（title属性、
       　なければ表示文字）を音声で読み上げる（本体クリックとは独立しており、
       　誤ってボタン自体を押してしまうことはない）。コマンドパレットで
       　「/all command」等を実行したときに表示される、コマンドの構文・説明の
       　各行にも同様に🔊バッジが付き、その行をそのまま読み上げられる。
       　新しく追加されるダイアログやリスト（DLC一覧・変更ログなど）にも
       　自動で付与される。オン/オフはツールバーの「🔊ボタン読み上げ」で
       　いつでも切り替えられ、初期状態はオン。
...
(function(){
  ...
  whenAppReady(function(){
    ...
    style.textContent = [
      ...
      'max-width:min(60vw,900px);overflow:auto;flex-shrink:0;}',
      ...
      '.dlc-pin-stage-wrap{overflow:auto;max-height:calc(100vh - 230px);border:1px solid #eee2d0;',
      'background:#eee9dc;}',
      ...
      'outline:1px dashed transparent;border-radius:2px;padding:2px 3px;}',
      '.dlc-pin-textbox:hover,.dlc-pin-textbox:focus{outline-color:rgba(62,110,99,.5);background:rgba(62,110,99,.05);}',
      '.dlc-pin-textbox-del{position:absolute;top:-9px;right:-9px;width:18px;height:18px;border-radius:50%;',
      'border:1px solid #DAD3C4;background:#fff;color:#B5432E;font-size:10px;line-height:1;cursor:pointer;',
      'display:none;align-items:center;justify-content:center;padding:0;}',
      '.dlc-pin-textbox-holder:hover .dlc-pin-textbox-del{display:flex;}',
      '.dlc-pin-empty{font-size:12px;color:#5B6270;text-align:center;padding:30px 10px;}',
      '.dlc-speak-badge{position:absolute;top:-7px;right:-7px;width:17px;height:17px;border-radius:50%;',
      'background:#3E6E63;color:#fff;font-size:9.5px;display:flex;align-items:center;justify-content:center;',
      'cursor:pointer;z-index:6;box-shadow:0 1px 3px rgba(0,0,0,.35),0 0 0 2px #fff;line-height:1;',
      'user-select:none;transition:transform .1s,background .15s;}',
      '.dlc-speak-badge:hover{background:#2c5148;transform:scale(1.12);}',
      '.dlc-speak-badge:active{transform:scale(0.88);}',
      '.dlc-speak-badge:focus-visible{outline:2px solid #B5432E;outline-offset:1px;}'
    ].join('');
    document.head.appendChild(style);

    var brand = document.querySelector('.brand-text h1');
    if (brand) brand.insertAdjacentHTML('beforeend', ' <span class="dlc-badge">📖 ディスレクシア支援パック v0.5</span>');
    ...
    pinBtn.title = '今表示しているページを固定表示する（もう一度押すと解除）。固定中は右側に見開き表示され、ペンでの手書きやテキストボックスの追加もできます。';
    ...
    function dlcPinStrokeSegment(ctx){
    ...
    function renderPinnedPane(){
      ...
      /* 【不具合修正】以前は中央寄せ(justify-content:center)のまま固定ページを右に
         追加していたため、幅が収まりきらないと、左側にあるはずの「今作業している
         ページ」の方が画面外（左マイナス方向）に隠れてしまい、逆に固定側だけが
         大きく見える状態になっていた。左詰め表示に変更したうえで、常に横スクロール
         位置を先頭へ戻し、今のページが必ず画面内に見えるようにする。 */
      if (canvasArea) canvasArea.scrollLeft = 0;
      ...
      pinnedPane.innerHTML =
        ...
        '</div></div>' +
        '<div class="dlc-pin-hint">「ペン」で手書き、「文字」でタップした場所にテキストボックスを追加できます。内容は通常表示にもそのまま反映されます。</div>';
      ...
      (p.textBoxes || []).forEach(function(tb){
        ...
        box.style.left = tb.x + 'px'; box.style.top = tb.y + 'px';
        box.style.width = tb.w + 'px'; box.style.height = tb.h + 'px';
        ...
        box.addEventListener('pointerdown', function(e){ if (dlcPinTool === 'select') e.stopPropagation(); });
        var delBtn = document.createElement('button');
        delBtn.type = 'button';
        delBtn.className = 'dlc-pin-textbox-del';
        delBtn.textContent = '✕';
        delBtn.title = 'このテキストボックスを削除';
        ...
        box.style.left = '0'; box.style.top = '0'; box.style.position = 'static';
      ...
      pinnedPane.querySelectorAll('.dlc-pin-tool-btn').forEach(function(btn){
        ...
        btn.addEventListener('click', function(){
          ...
          stage.style.cursor = dlcPinTool === 'pen' ? 'crosshair' : (dlcPinTool === 'text' ? 'copy' : 'default');
        });
      });
      stage.style.cursor = dlcPinTool === 'pen' ? 'crosshair' : (dlcPinTool === 'text' ? 'copy' : 'default');
    ...
    function dlcLabelForElement(el){
      ...
      if (clone.querySelectorAll){
        clone.querySelectorAll('.dlc-speak-badge').forEach(function(b){ b.remove(); });
      }
      return (clone.textContent || '').replace(/\s+/g, ' ').trim();
    }

    function dlcSpeakBadgeClick(e){
      e.stopPropagation();
      if (e.preventDefault) e.preventDefault();
      var host = e.currentTarget && e.currentTarget.parentElement;
      if (!host) return;
      dlcSpeakLabel(dlcLabelForElement(host));
    }

    function makeSpeakBadge(){
      var b = document.createElement('span');
      b.className = 'dlc-speak-badge';
      b.setAttribute('role', 'button');
      b.setAttribute('tabindex', '0');
      b.setAttribute('aria-label', '読み上げる');
      b.textContent = '🔊';
      b.addEventListener('click', dlcSpeakBadgeClick);
      b.addEventListener('keydown', function(e){
        if (e.key === 'Enter' || e.key === ' '){ dlcSpeakBadgeClick(e); }
      });
      return b;
    }
    ...
    function attachSpeakBadge(el){
      ...
      if (el.classList && el.classList.contains('dlc-speak-badge')) return;
      el.dataset.dlcSpoken = '1';
      try {
        var cs = window.getComputedStyle(el);
        if (cs && cs.position === 'static') el.style.position = 'relative';
      } catch(e){}
      el.appendChild(makeSpeakBadge());
    }
    ...
    function removeAllSpeakBadges(){
      document.querySelectorAll('.dlc-speak-badge').forEach(function(b){ b.remove(); });
      document.querySelectorAll('[data-dlc-spoken]').forEach(function(el){ delete el.dataset.dlcSpoken; });
    }
    ...
    speakToggleBtn.title = '全てのボタンやコマンド構文の行に小さな🔊読み上げバッジを表示し、押すとその名前や説明を読み上げます。もう一度押すとオフにできます（初期状態はオン）。';
    ...
    dlcLog('ディスレクシア支援パックを有効化', 'v0.5');

    console.log('[読みノート] ディスレクシア支援パック v0.5 を有効化しました。');
  });
})();
