/* ============================================================================
   ...
   <script>...</script> より「後」に以下のように読み込んでください。
     ...
     <script src="dyslexia-support-pack-v0.6.js"></script>
     </body>
   ...
   ▼ 新しく使えるようになるツール
     ...
     ・ページのピン止め（📌／v0.6で見やすさ・使いやすさを大幅改善）
       　今見ているページを「固定表示」しておくと、以降ページを移動しても
       　固定したページが画面右側にサイドパネルとして表示され続ける。
       ...
       　リンクもそのまま表示される。テキストボックスはその場で編集でき、認識
       　済みブロックはクリックで読み上げられる。
     ・UDフォント（Aa）
...
(function(){
  ...
  whenAppReady(function(){
    ...
    if (brand) brand.insertAdjacentHTML('beforeend', ' <span class="dlc-badge">📖 ディスレクシア支援パック v0.6</span>');
    ...
    dlcLog('ディスレクシア支援パックを有効化', 'v0.6');

    console.log('[読みノート] ディスレクシア支援パック v0.6 を有効化しました。');
  });
})();
