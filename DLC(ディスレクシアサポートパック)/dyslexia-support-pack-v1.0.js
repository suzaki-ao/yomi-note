/* ============================================================================
   ...
   <script>...</script> より「後」に以下のように読み込んでください。
     ...
     <script src="dyslexia-support-pack-v1.0.js"></script>
     </body>
   ...
   ▼ 新しく使えるようになるツール
     ...
     ・ページのピン止め（📌／v0.6-v0.7で見やすさ・使いやすさを大幅改善）
       ...
       　はクリックで読み上げられる。
     ・大文字表示（🔠／v1.0で新規追加）
       　テキストボックスの文字や操作画面のボタン・見出しを、通常より大きく・
       　太く表示して見やすくする。もう一度押すと元の大きさに戻る。
       　（v1.0でUDフォント切り替えボタンは廃止。本体アプリが標準でUDフォント
       　「BIZ UDゴシック」を使うようになったため、切り替える必要が無くなった）
     ・ログ（📝）
   ...
   反映される。
   v0.9の変更点：【ピン止め機能を超強化】これまで1ページしか固定できず、別のページを
   固定すると前のものが自動的に解除されていた不便を解消。複数のページを同時に固定でき、
   パネル上部に表示されるタブをクリックして切り替えられるようにした（タブの×で個別に
   固定解除も可能）。1ページだけ固定しているときは、これまで通りタブは表示されない。
   【OCR強化】部分OCR縦+／横+が使う高精度ブースト設定を本体側で強化（拡大率・目標
   サイズを引き上げ、カメラ撮影ページでは陰影ムラの補正も自動で有効化）。本体v4.2との
   組み合わせで効果が出る。
   v1.0の変更点（バージョン番号の付け方を整理し、v0.9の次はv1.0としました）：
   【機能廃止】UDフォント（Aa）の切り替えボタンを削除。本体アプリ v4.3 から標準で
   常時「BIZ UDゴシック」フォントを使うようになったため、このDLC側で個別に切り替える
   必要が無くなったための整理。
   【新機能】「🔠大文字表示」を追加。テキストボックスの文字や操作画面のボタン・見出しを
   通常より大きく・太く表示して見やすくする。もう一度押すと元に戻る。
   ============================================================================ */
(function(){
  'use strict';
  ...
  whenAppReady(function(){
    ...
    style.textContent = [
      ...
      /* 【v0.9で追加】複数ページ同時固定タブ */
      '.dlc-pin-tabs{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:8px;}',
      '.dlc-pin-tab{display:flex;align-items:center;gap:5px;font-size:11px;background:#FBF8F2;',
      'border:1px solid #DAD3C4;border-radius:12px;padding:3px 6px 3px 10px;cursor:pointer;color:#5B6270;}',
      '.dlc-pin-tab.active{background:#22262E;color:#fff;border-color:#22262E;}',
      '.dlc-pin-tab-close{border:none;background:transparent;color:inherit;cursor:pointer;',
      'font-size:12px;line-height:1;padding:0 2px;opacity:.7;}',
      '.dlc-pin-tab-close:hover{opacity:1;}',
    ...
    if (brand) brand.insertAdjacentHTML('beforeend', ' <span class="dlc-badge">📖 ディスレクシア支援パック v1.0</span>');
    ...
    /* 【v0.9で追加：ピン止め機能の大幅強化】
       これまでは一度に1ページしか固定できず、別のページを固定すると前のものは
       自動的に解除されていた。複数のページを同時に固定し、パネル上部のタブで
       切り替えられるようにする。dlcPinnedPageIndex は「今パネルに表示中の
       ページ」を指す値として引き続き使い（既存コードの大部分をそのまま活かす
       ため）、新しく dlcPinnedIndices 配列で「固定中のページ一覧」を管理する。 */
    window.dlcPinnedIndices = [];
    window.dlcPinZoom = 1;
    ...
    pinBtn.title = '今表示しているページを固定表示に追加する（もう一度押すと解除）。複数ページを同時に固定でき、パネル上部のタブで切り替えられます。幅はドラッグで変更でき、今作業中の普通のページ側のツールもそのまま使えます。';
    pinBtn.innerHTML = '<span class="glyph">📌</span>ピン止め';
    pinBtn.addEventListener('click', function(){
      var pos = window.dlcPinnedIndices.indexOf(currentPageIndex);
      if (pos !== -1){
        /* すでに固定済みなら解除。今表示中のタブがこれだった場合は、
           残っている固定ページのうち近いものへ自動で切り替える。 */
        window.dlcPinnedIndices.splice(pos, 1);
        if (window.dlcPinnedPageIndex === currentPageIndex){
          window.dlcPinnedPageIndex = window.dlcPinnedIndices.length
            ? window.dlcPinnedIndices[Math.min(pos, window.dlcPinnedIndices.length - 1)]
            : null;
        }
      } else {
        window.dlcPinnedIndices.push(currentPageIndex);
    ...
    function updatePinButtonUI(){
      pinBtn.classList.toggle('dlc-forced-active', window.dlcPinnedIndices.indexOf(currentPageIndex) !== -1);
    }
    ...
    function renderPinnedPane(){
      ...
      /* 固定後にページが削除された等で範囲外になった項目は取り除く（安全対策） */
      window.dlcPinnedIndices = (window.dlcPinnedIndices || []).filter(function(i){ return !!notestate.pages[i]; });
      if (window.dlcPinnedPageIndex !== null && window.dlcPinnedIndices.indexOf(window.dlcPinnedPageIndex) === -1){
        window.dlcPinnedPageIndex = window.dlcPinnedIndices.length ? window.dlcPinnedIndices[0] : null;
      }
      ...
      /* 【v0.9で追加】複数固定しているときだけ、パネル上部にタブを表示して
         切り替えられるようにする。1件だけの場合は今まで通りタブなし。 */
      var tabsHtml = '';
      if (window.dlcPinnedIndices.length > 1){
        tabsHtml = '<div class="dlc-pin-tabs">' + window.dlcPinnedIndices.map(function(i){
          return '<span class="dlc-pin-tab' + (i === idx ? ' active' : '') + '" data-pin-tab="' + i + '">' +
            'ページ' + (i + 1) +
            '<button type="button" class="dlc-pin-tab-close" data-pin-tab-close="' + i + '" title="このページの固定を解除">×</button>' +
          '</span>';
        }).join('') + '</div>';
      }

      pinContent.innerHTML =
        '<div class="dlc-pin-label"><span class="dlc-pin-label-text">📌 ページ ' + (idx + 1) + ' を固定表示中' +
          (window.dlcPinnedIndices.length > 1 ? '（他' + (window.dlcPinnedIndices.length - 1) + '件も固定中）' : '') + '</span>' +
        '<span>' +
        ...
        '</div></div>' +
        '<div class="dlc-pin-hint">「ペン」で手書き、「消す」でその部分の手書きを消去、「文字」でタップした場所にテキストボックスを追加できます。内容は通常表示にもそのまま反映されます。パネル左端をドラッグすると幅を変更できます。' +
          (window.dlcPinnedIndices.length > 1 ? '上部のタブで固定中の他のページに切り替えられます。' : '') +
        '</div>';

      pinnedPane.querySelectorAll('[data-pin-tab]').forEach(function(tab){
        tab.addEventListener('click', function(e){
          if (e.target.closest('[data-pin-tab-close]')) return;
          window.dlcPinnedPageIndex = parseInt(tab.getAttribute('data-pin-tab'), 10);
          var pNew = notestate.pages[window.dlcPinnedPageIndex];
          if (pNew) window.dlcPinZoom = computeDlcPinFitZoom(pNew);
          renderPinnedPane();
        });
      });
      pinnedPane.querySelectorAll('[data-pin-tab-close]').forEach(function(btn){
        btn.addEventListener('click', function(e){
          e.stopPropagation();
          var closeIdx = parseInt(btn.getAttribute('data-pin-tab-close'), 10);
          var pos = window.dlcPinnedIndices.indexOf(closeIdx);
          if (pos !== -1) window.dlcPinnedIndices.splice(pos, 1);
          if (window.dlcPinnedPageIndex === closeIdx){
            window.dlcPinnedPageIndex = window.dlcPinnedIndices.length
              ? window.dlcPinnedIndices[Math.min(pos, window.dlcPinnedIndices.length - 1)]
              : null;
          }
      ...
      if (unpinBtn){
        unpinBtn.addEventListener('click', function(){
          var pos = window.dlcPinnedIndices.indexOf(window.dlcPinnedPageIndex);
          if (pos !== -1) window.dlcPinnedIndices.splice(pos, 1);
          window.dlcPinnedPageIndex = window.dlcPinnedIndices.length
            ? window.dlcPinnedIndices[Math.min(pos, window.dlcPinnedIndices.length - 1)]
            : null;
    ...
    window.addEventListener('resize', function(){ if (window.dlcPinnedIndices && window.dlcPinnedIndices.length) renderPinnedPane(); });
    ...
    /* ==========================================================
       5. UDフォント切り替えツールについて
       【v1.0で削除】これまでこのDLCが提供していた「UDフォント（Aa）」の
       オン/オフ切り替えボタンは、本体アプリ v4.3 から常時「BIZ UDゴシック」を
       標準フォントとして使うようになったため、このDLC側の切り替えボタンは
       廃止した（本体が既にUDフォントなので、わざわざ切り替える必要が無い）。
    ========================================================== */

    /* ==========================================================
       6. 大文字表示（v1.0で新規追加）
       文字を通常より大きく・太く表示することで見やすくする機能。
       ノートのテキストボックスの文字、および操作画面のボタン・見出し類を
       対象に、フォントサイズを引き上げ、太字にする。ページの背景画像
       （PDF/写真そのもの）の文字の大きさは変わらないが、テキストボックスや
       画面の文字は大きくはっきり見えるようになる。
    ========================================================== */
    var bigTextStyle = document.createElement('style');
    bigTextStyle.textContent =
      "html.dlc-big-text body{font-size:113% !important;}" +
      "html.dlc-big-text .textbox{font-weight:700 !important;}" +
      "html.dlc-big-text button,html.dlc-big-text .tool-btn,html.dlc-big-text .btn," +
      "html.dlc-big-text .icon-btn{font-weight:700 !important;}" +
      "html.dlc-big-text .sidebar-section h3,html.dlc-big-text .panel-row,html.dlc-big-text label," +
      "html.dlc-big-text #sentencePreview{font-weight:700 !important;}" +
      "html.dlc-big-text .dlc-pin-stage,html.dlc-big-text .dlc-pin-hint{font-weight:700 !important;}";
    document.head.appendChild(bigTextStyle);

    var BIG_TEXT_KEY = 'yomiDlcBigTextEnabled';
    function applyBigTextState(){
      var on = false;
      try { on = localStorage.getItem(BIG_TEXT_KEY) === 'true'; } catch(e){}
      document.documentElement.classList.toggle('dlc-big-text', on);
      return on;
    }
    var bigTextBtn = document.createElement('button');
    bigTextBtn.type = 'button';
    bigTextBtn.className = 'tool-btn';
    bigTextBtn.id = 'dlcBigTextBtn';
    bigTextBtn.title = '画面の文字を通常より大きく・太く表示して見やすくします。もう一度押すと元に戻ります。';
    bigTextBtn.innerHTML = '<span class="glyph">🔠</span>大文字表示';
    bigTextBtn.addEventListener('click', function(){
      var wasOn = document.documentElement.classList.contains('dlc-big-text');
      var next = !wasOn;
      try { localStorage.setItem(BIG_TEXT_KEY, next ? 'true' : 'false'); } catch(e){}
      document.documentElement.classList.toggle('dlc-big-text', next);
      bigTextBtn.classList.toggle('dlc-forced-active', next);
      if (typeof flashToast === 'function') flashToast(next ? '大文字表示にしました' : '通常の文字の大きさに戻しました');
      dlcLog('大文字表示切替', next ? 'オンにした' : 'オフにした');
    });
    if (toolGrid) toolGrid.appendChild(bigTextBtn);
    bigTextBtn.classList.toggle('dlc-forced-active', applyBigTextState());
    ...
    console.log('[読みノート] ディスレクシア支援パック v1.0 を有効化しました。');
  });
})();
