/* ============================================================================
   ...
   ▼ 対応言語
     ...
     ・Pythonはブラウザ内実行エンジン(Pyodide)を初回実行時に読み込んで実行します
       （初回はインターネット接続が必要です）。
     ・C言語系はv0.2からブラウザ内実行に対応（簡易インタプリタJSCPPを使用）。
   ...
   v0.2の変更点：【大幅強化】C/C++の実行に対応（JSCPP、MITライセンスのJavaScript製
   簡易インタプリタをCDNから読み込んで使用）。printf/coutでの出力、if/for/while、
   配列、関数など標準的な書き方をコンパイル不要でその場で実行できるようになった
   （本物のgcc/clang相当ではなく簡易インタプリタのため、対応していない書き方も
   一部ある点はご了承ください）。
   ============================================================================ */
(function(){
  'use strict';
  ...
  whenAppReady(function(){
    ...
    root.innerHTML =
      '<div id="progDlcTopbar">' +
        '<span class="pd-title">🧑‍💻 プログラミング追加パック v0.2</span>' +
    ...
    /* 【v0.2で新規】C/C++実行機能。これまでは編集・保存のみで、ブラウザ内での
       実行には対応していなかった。JSCPP（felixhao28氏によるJavaScript製の
       C++インタプリタ、MITライセンス）をCDNから読み込んで使うことで、標準的な
       C/C++の構文（printf/coutでの出力、if/for/while、配列、関数など）を
       サーバーなし・コンパイル不要でその場で実行できるようにした。
       ただし、あくまでJavaScriptで書かれた簡易インタプリタであり、本物の
       コンパイラ（gcc/clang等）と完全に同じ動作を保証するものではない
       （対応していない標準ライブラリ関数や書き方もある）点はご了承ください。 */
    var jscppReady = null;
    function ensureJscpp(){
      if (jscppReady) return jscppReady;
      if (window.JSCPP) return (jscppReady = Promise.resolve(window.JSCPP));
      log('C/C++実行環境を読み込んでいます…（初回のみ）');
      jscppReady = new Promise(function(resolve, reject){
        var s = document.createElement('script');
        s.src = 'https://felixhao28.github.io/JSCPP/dist/JSCPP.es5.min.js';
        s.onload = function(){
          if (window.JSCPP) resolve(window.JSCPP);
          else reject(new Error('C/C++実行環境の読み込みに失敗しました'));
        };
        s.onerror = function(){ reject(new Error('C/C++実行環境の読み込みに失敗しました（通信環境をご確認ください）')); };
        document.head.appendChild(s);
      });
      return jscppReady;
    }
    function runCProgram(f){
      showBottom('console');
      log('--- ' + f.name + ' を実行 ---', false);
      ensureJscpp().then(function(JSCPP){
        try {
          var exitCode = JSCPP.run(f.content, '', {
            stdio: { write: function(s){ log(s); } },
            unsigned_overflow: 'ignore',
            maxTimeout: 8000
          });
          log('（プログラムはコード ' + exitCode + ' で終了しました）', false);
        } catch(err){
          log('実行エラー： ' + (err && err.message ? err.message : String(err)), true);
          log('※このC/C++実行はJSCPP（簡易インタプリタ）によるものです。標準的な書き方の多くは動きますが、すべてのライブラリ関数・書き方に対応しているわけではありません。', false);
        }
      }).catch(function(err){
    ...
    renderTabs();
    renderTools();
    toast('🧑‍💻 プログラミング追加パック v0.2 が有効になりました');
  });
})();
